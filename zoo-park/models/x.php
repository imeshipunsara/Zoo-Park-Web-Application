...
//             $sql = "INSERT INTO educational_informations (name,information_text,cover_image) VALUES (:name, :information_text,:cover_image)";
...
//             $sql = "UPDATE educational_informations SET name = :name, information_text = :information_text, cover_image = :cover_image WHERE id = :id";
//             $stmt = $conn->prepare($sql);
//             $stmt->bindParam(':name', $this->name);
//             $stmt->bindParam(':information_text', $this->information_text);
//             $stmt->bindParam(':cover_image', $this->cover_image);
...
//             $sql = "DELETE FROM educational_informations WHERE id = :id";
//             $stmt = $conn->prepare($sql);
...
//     // get all information
//     public function get_all_edu_infos()
//     {
//         try {
//             // create new db connection object
//             $conn = DBConnection::GetConnection();

//             $sql = "SELECT * FROM educational_informations ORDER BY id DESC";
//             $stmt = $conn->prepare($sql);
//             $stmt->execute();
//             $rows = $stmt->fetchAll();
//             $edu_infos = array();
//             foreach ($rows as $row) {
//                 $item = new EducationalInfomation();
//                 $item->id = $row['id'];
//                 $item->name = $row['name'];
//                 $item->cover_image = $row['cover_image'];
//                 $item->information_text = $row['information_text'];
//                 array_push($edu_infos, $item);
//             }
...
//             $sql = "SELECT * FROM educational_informations WHERE id = :x";
//             $stmt = $conn->prepare($sql);
//             $stmt->bindParam(':x', $id);
//             $stmt->execute();
//             $row = $stmt->fetch();
